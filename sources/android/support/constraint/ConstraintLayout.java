            ...
            while (i < sizeDependentWidgetsCount2) {
                ...
                if ((widget instanceof android.support.constraint.solver.widgets.Guideline) || (child = (View) widget.getCompanionWidget()) == null) {
                    ...
                    if (child.getVisibility() != 8) {
                        ...
                        int widthSpec = params.width == -2 ? getChildMeasureSpec(widthMeasureSpec, widthPadding, params.width) : View.MeasureSpec.makeMeasureSpec(widget.getWidth(), 1073741824);
                        int heightSpec = params.height == -2 ? getChildMeasureSpec(heightMeasureSpec, heightPadding, params.height) : View.MeasureSpec.makeMeasureSpec(widget.getHeight(), 1073741824);
                        child.measure(widthSpec, heightSpec);
                        int measuredWidth = child.getMeasuredWidth();
                        int measuredHeight = child.getMeasuredHeight();
                        int heightSpec2 = widget.getWidth();
                        if (measuredWidth != heightSpec2) {
                            widget.setWidth(measuredWidth);
                            if (!containerWrapWidth2 || widget.getRight() <= this.mLayoutWidget.getWidth()) {
