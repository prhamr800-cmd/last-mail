...
public abstract class PrimitiveSpreadBuilder<T> {
    ...
    protected final T toArray(@NotNull T values, @NotNull T result) {
        ...
        if (i2 >= 0) {
            ...
            int dstIndex2 = 0;
            while (true) {
                T t = this.spreads[dstIndex2];
                if (t != null) {
                    if (copyValuesFrom < dstIndex2) {
                        System.arraycopy(values, copyValuesFrom, result, dstIndex, dstIndex2 - copyValuesFrom);
                        dstIndex += dstIndex2 - copyValuesFrom;
                    }
                    int spreadSize = getSize(t);
                    System.arraycopy(t, 0, result, dstIndex, spreadSize);
                    dstIndex += spreadSize;
                    copyValuesFrom = dstIndex2 + 1;
                }
                if (dstIndex2 == i2) {
                    break;
                }
        ...
        if (copyValuesFrom < this.size) {
            System.arraycopy(values, copyValuesFrom, result, i, this.size - copyValuesFrom);
        }
        return result;
    }
}
