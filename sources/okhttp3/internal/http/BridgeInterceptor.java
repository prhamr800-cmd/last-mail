...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (userRequest.header("Host") == null) {
            requestBuilder.header("Host", okhttp3.internal.Util.hostHeader(userRequest.url(), false));
        }
        ...
        if (transparentGzip && HttpRequest.ENCODING_GZIP.equalsIgnoreCase(networkResponse.header(HttpRequest.HEADER_CONTENT_ENCODING)) && HttpHeaders.hasBody(networkResponse)) {
            ...
            responseBuilder.body(new RealResponseBody(networkResponse.header(HttpRequest.HEADER_CONTENT_TYPE), -1L, Okio.buffer(responseBody)));
        }
