...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        int i6 = i;
        updateAllRemainingSpans(layoutState.mLayoutDirection, i6);
        if (this.mShouldReverseLayout) {
            startAfterPadding = this.mPrimaryOrientation.getEndAfterPadding();
        } else {
            startAfterPadding = this.mPrimaryOrientation.getStartAfterPadding();
            ...
            if (layoutState.hasMore(state) && (this.mLayoutState.mInfinite || !this.mRemainingSpans.isEmpty())) {
                ...
                if (layoutState.mLayoutDirection == r15) {
                    if (layoutParams2.mFullSpan) {
                        endLine = getMaxEnd(i7);
                    } else {
                        endLine = span3.getEndLine(i7);
                    }
                    int decoratedMeasurement3 = this.mPrimaryOrientation.getDecoratedMeasurement(next) + endLine;
                    if (z3 && layoutParams2.mFullSpan) {
                        LazySpanLookup.FullSpanItem fullSpanItemCreateFullSpanItemFromEnd = createFullSpanItemFromEnd(endLine);
                    ...
                    if (layoutParams2.mFullSpan) {
                        startLine = getMinStart(i7);
                    } else {
                        startLine = span3.getStartLine(i7);
                    }
                    int decoratedMeasurement4 = startLine - this.mPrimaryOrientation.getDecoratedMeasurement(next);
                    if (z3 && layoutParams2.mFullSpan) {
                        LazySpanLookup.FullSpanItem fullSpanItemCreateFullSpanItemFromStart = createFullSpanItemFromStart(startLine);
                ...
                if (isLayoutRTL() && this.mOrientation == r15) {
                    int endAfterPadding = layoutParams2.mFullSpan ? this.mSecondaryOrientation.getEndAfterPadding() : this.mSecondaryOrientation.getEndAfterPadding() - (((this.mSpanCount - r15) - span3.mIndex) * this.mSizePerSpan);
                    decoratedMeasurement2 = endAfterPadding;
                    decoratedMeasurement = endAfterPadding - this.mSecondaryOrientation.getDecoratedMeasurement(next);
                } else {
                    if (layoutParams2.mFullSpan) {
                ...
                if (this.mOrientation == r15) {
                    ...
                    i4 = i7;
                    i5 = i6;
                    layoutDecoratedWithMargins(next, i3, decoratedMeasurement, i2, decoratedMeasurement2);
                }
        ...
        int i8 = maxEnd;
        if (i8 > 0) {
            return Math.min(layoutState.mAvailable, i8);
        }
        return 0;
    }
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (!preferLastSpan(layoutDir)) {
            while (true) {
                ...
                View unfocusableCandidate2 = findViewByPosition(iFindLastPartiallyVisibleItemPosition);
            ...
            while (true) {
                ...
                if (i7 != prevFocusSpan.mIndex) {
                    if (shouldSearchFromStart) {
                        iFindLastPartiallyVisibleItemPosition2 = this.mSpans[i7].findFirstPartiallyVisibleItemPosition();
                    } else {
                        iFindLastPartiallyVisibleItemPosition2 = this.mSpans[i7].findLastPartiallyVisibleItemPosition();
                    }
                    View unfocusableCandidate3 = findViewByPosition(iFindLastPartiallyVisibleItemPosition2);
