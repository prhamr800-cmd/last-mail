...
class RandomBackoff implements Backoff {
    ...
    public long getDelayMillis(int retries) {
        double dRandomJitter = randomJitter();
        double delayMillis = this.backoff.getDelayMillis(retries);
        Double.isNaN(delayMillis);
        return (long) (dRandomJitter * delayMillis);
    }

    double randomJitter() {
        double minJitter = 1.0d - this.jitterPercent;
        double maxJitter = this.jitterPercent + 1.0d;
