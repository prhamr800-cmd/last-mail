...
final class DetectionResultRowIndicatorColumn extends DetectionResultColumn {
    ...
    void adjustCompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodeMetadata) {
        ...
        int maxRowHeight = 1;
        int barcodeRow = -1;
        int barcodeRow2 = firstRow;
        while (barcodeRow2 < lastRow) {
            if (codewords[barcodeRow2] != null) {
                Codeword codeword = codewords[barcodeRow2];
                ...
                if (rowDifference == 0) {
                    ...
                    currentRowHeight = 1;
                } else if (rowDifference < 0 || codeword.getRowNumber() >= barcodeMetadata.getRowCount() || rowDifference > barcodeRow2) {
                    codewords[barcodeRow2] = null;
                } else {
                    int checkedRows = maxRowHeight > 2 ? (maxRowHeight - 2) * rowDifference : rowDifference;
                    boolean closePreviousCodewordFound = checkedRows >= barcodeRow2;
                    boolean closePreviousCodewordFound2 = closePreviousCodewordFound;
                    for (int i = 1; i <= checkedRows && !closePreviousCodewordFound2; i++) {
                        closePreviousCodewordFound2 = codewords[barcodeRow2 - i] != null;
                    }
                    if (closePreviousCodewordFound2) {
                        codewords[barcodeRow2] = null;
                    } else {
                        barcodeRow = codeword.getRowNumber();
                        currentRowHeight = 1;
    ...
    private void adjustIncompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodeMetadata) {
        ...
        int maxRowHeight = 1;
        int barcodeRow = -1;
        for (int barcodeRow2 = firstRow; barcodeRow2 < lastRow; barcodeRow2++) {
            if (codewords[barcodeRow2] != null) {
                Codeword codeword = codewords[barcodeRow2];
                codeword.setRowNumberAsRowIndicatorColumn();
                int rowDifference = codeword.getRowNumber() - barcodeRow;
