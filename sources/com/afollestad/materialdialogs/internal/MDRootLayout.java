...
public class MDRootLayout extends ViewGroup {
    ...
    private static boolean isVisible(View v) {
        ...
        if (visible && (v instanceof MDButton)) {
            return ((MDButton) v).getText().toString().trim().length() > 0;
        }
        return visible;
    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.stackBehavior == StackingBehavior.ALWAYS) {
            ...
            int buttonFrameWidth = width - (getContext().getResources().getDimensionPixelSize(R.dimen.md_neutral_button_margin) * 2);
        ...
        if (stacked) {
            ...
            int stackedHeight3 = buttonBarPadding;
        ...
        if (isVisible(this.content)) {
            this.content.measure(View.MeasureSpec.makeMeasureSpec(width, 1073741824), View.MeasureSpec.makeMeasureSpec(availableHeight - minPadding, Integer.MIN_VALUE));
            if (this.content.getMeasuredHeight() <= availableHeight - fullPadding) {
                if (!this.reducePaddingNoTitleNoButtons || isVisible(this.titleBar) || buttonBarPadding != 0) {
                    this.useFullPadding = true;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (isVisible(this.content)) {
            this.content.layout(l, t2, r, this.content.getMeasuredHeight() + t2);
        }
        if (this.isStacked) {
    ...
    public void invalidateDividersForScrollingView(ViewGroup view, boolean setForTop, boolean setForBottom, boolean hasButtons) {
        if (setForTop && view.getChildCount() > 0) {
            this.drawTopDivider = (this.titleBar == null || this.titleBar.getVisibility() == 8 || view.getScrollY() + view.getPaddingTop() <= view.getChildAt(0).getTop()) ? false : true;
        }
    ...
    public void invalidateDividersForWebView(WebView view, boolean setForTop, boolean setForBottom, boolean hasButtons) {
        if (setForTop) {
            this.drawTopDivider = (this.titleBar == null || this.titleBar.getVisibility() == 8 || view.getScrollY() + view.getPaddingTop() <= 0) ? false : true;
        }
        if (setForBottom) {
