...
final class MatrixUtil {
    ...
    static void embedTypeInfo(ErrorCorrectionLevel ecLevel, int maskPattern, ByteMatrix matrix) throws WriterException {
        ...
        for (int i = 0; i < typeInfoBits.getSize(); i++) {
            ...
            if (i < 8) {
                int x22 = matrix.getWidth();
                x2 = (x22 - i) - 1;
                matrix.set(x2, 8, bit);
            } else {
                int x23 = x2;
                int x24 = matrix.getHeight();
                int y2 = (x24 - 7) + (i - 8);
                matrix.set(8, y2, bit);
                x2 = x23;
            }
    ...
    static void maybeEmbedVersionInfo(Version version, ByteMatrix matrix) throws WriterException {
        ...
        int bitIndex2 = 0;
        while (bitIndex2 < 6) {
            int bitIndex3 = bitIndex;
            for (int bitIndex4 = 0; bitIndex4 < 3; bitIndex4++) {
                boolean bit = versionInfoBits.get(bitIndex3);
                bitIndex3--;
                matrix.set(bitIndex2, (matrix.getHeight() - 11) + bitIndex4, bit);
                matrix.set((matrix.getHeight() - 11) + bitIndex4, bitIndex2, bit);
            }
            bitIndex2++;
            bitIndex = bitIndex3;
        }
    ...
    static void embedDataBits(BitArray dataBits, int maskPattern, ByteMatrix matrix) throws WriterException {
        ...
        while (x > 0) {
            ...
            while (y >= 0 && y < matrix.getHeight()) {
                ...
                for (int bitIndex3 = 0; bitIndex3 < 2; bitIndex3++) {
                    int xx = x - bitIndex3;
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix matrix) {
        ...
        while (i < numCoordinates) {
            ...
            for (int j = 0; j < numCoordinates; j++) {
                int y = coordinates[i];
                int i3 = coordinates[j];
                x = i3;
                if (i3 != -1 && y != -1 && isEmpty(matrix.get(x, y))) {
