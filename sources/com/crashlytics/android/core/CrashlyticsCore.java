...
public class CrashlyticsCore extends Kit<Void> {
    ...
    boolean onPreExecute(Context context) {
        ...
        if (!isBuildIdValid(this.buildId, requiresBuildId)) {
        ...
        try {
            ...
            if (!installExceptionHandler(unityVersionProvider)) {
                return false;
            }
    ...
    private void checkForPreviousCrash() {
        Boolean foundPreviousCrash = (Boolean) this.executorServiceWrapper.executeSyncLoggingException(new CrashMarkerCheck(this.crashMarker));
    ...
    public boolean getSendDecisionFromUser(Activity activity, PromptSettingsData promptData) {
        CrashPromptDialog.AlwaysSendCallback alwaysSendCallback = new CrashPromptDialog.AlwaysSendCallback() { // from class: com.crashlytics.android.core.CrashlyticsCore.7
            ...
            public void sendUserReportsWithoutPrompting(boolean send) {
                CrashlyticsCore.this.setShouldSendUserReportsWithoutPrompting(send);
            }
        };
        final CrashPromptDialog dialog = CrashPromptDialog.create(activity, promptData, alwaysSendCallback);
