...
class SessionProtobufHelper {
    ...
    private static void writeSessionEventAppExecutionException(CodedOutputStream cos, TrimmedThrowableData exception, int chainDepth, int maxChainedExceptionsDepth, int field) throws Exception {
        ...
        StackTraceElement[] arr$ = exception.stacktrace;
        int overflowCount = 0;
    ...
    private static int getEventAppExecutionSize(TrimmedThrowableData exception, Thread exceptionThread, StackTraceElement[] exceptionStack, Thread[] otherThreads, List<StackTraceElement[]> otherStacks, int maxChainedExceptionDepth, ByteString packageNameBytes, ByteString optionalBuildIdBytes) {
        ...
        int len = otherThreads.length;
        int size2 = size;
        for (int size3 = 0; size3 < len; size3++) {
            Thread thread = otherThreads[size3];
            int threadSize2 = getThreadSize(thread, otherStacks.get(size3), 0, false);
            size2 += CodedOutputStream.computeTagSize(1) + CodedOutputStream.computeRawVarint32Size(threadSize2) + threadSize2;
        }
        int exceptionSize = getEventAppExecutionExceptionSize(exception, 1, maxChainedExceptionDepth);
        int size4 = size2 + CodedOutputStream.computeTagSize(2) + CodedOutputStream.computeRawVarint32Size(exceptionSize) + exceptionSize;
        int signalSize = getEventAppExecutionSignalSize();
        int size5 = size4 + CodedOutputStream.computeTagSize(3) + CodedOutputStream.computeRawVarint32Size(signalSize) + signalSize;
        int binaryImageSize = getBinaryImageSize(packageNameBytes, optionalBuildIdBytes);
        return size5 + CodedOutputStream.computeTagSize(3) + CodedOutputStream.computeRawVarint32Size(binaryImageSize) + binaryImageSize;
    }
    ...
    private static int getEventAppExecutionExceptionSize(TrimmedThrowableData ex, int chainDepth, int maxChainedExceptionsDepth) {
        ...
        StackTraceElement[] arr$ = ex.stacktrace;
        int overflowCount = 0;
        int size2 = size;
        for (StackTraceElement element : arr$) {
        ...
        if (cause != null) {
            ...
            while (true) {
                ...
                if (cause != null) {
                    ...
                    return size2 + CodedOutputStream.computeUInt32Size(7, overflowCount2);
                }
            }
        } else {
            return size2;
