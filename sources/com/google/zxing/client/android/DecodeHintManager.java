                ...
                if (parameterText != null) {
                    if (hintType.getValueType().equals(Object.class)) {
                        ...
                        if (!parameterText.isEmpty() && parameterText.charAt(parameterText.length() - 1) == ',') {
                            parameterText = parameterText.substring(i, parameterText.length() - 1);
                        }
                        ...
                        while (true) {
                            int i4 = i3;
                            int i5 = values.length;
                            if (i4 >= i5) {
                                break;
                            }
                            try {
                                array[i4] = Integer.parseInt(values[i4]);
                                i3 = i4 + 1;
                            } catch (NumberFormatException e) {
                                Log.w(TAG, "Skipping array of integers hint " + hintType + " due to invalid numeric value: '" + values[i4] + '\'');
                                array = null;
                            }
                        }
                        if (array != null) {
